...
<div class="">
	...
	<div class="headImgBox" :style="{backgroundImage:this.$store.state.themeObj.top_image?'url('+this.$store.state.themeObj.top_image+')':'url(static/img/img02.jpg)'}">
		<div class="scene">
			<div><span id="luke"></span></div>
		</div>
		<div class="h-information">

                    <img :src="this.$store.state.themeObj.head_portrait?this.$store.state.themeObj.head_portrait:'static/img/1.png'" alt="">
...
export default {
	...
	mounted() { //页面元素加载完成
		...
		var timer = setTimeout(function() {
			Typeit(that.$store.state.themeObj.usert_start, "#luke"); //打字机效果
			clearTimeout(timer);
		}, 500);
	}
}
